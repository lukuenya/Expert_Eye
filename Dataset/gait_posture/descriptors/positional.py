...
def quotient_both_direction(signal, axis=labels.MLAP):
    ...
    if amplitude_ap == 0:
        print('Warning: Division by zero encountered in quotient_both_direction function for axis', axis)
        feature = np.nan  # Use NaN as the value when division by zero is encountered
    else:
